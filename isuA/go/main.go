...
func getJIAServiceURL(tx *sqlx.Tx) string {
	...
	return isuAssociationConfig.URL
}
...
func postInitialize(c echo.Context) error {
	...
	isuAssociationConfig = Config{Name: "jia_service_url", URL: request.JIAServiceURL}
...
func postIsu(c echo.Context) error {
	...
	err = tx.Get(
		&isu,
		"SELECT * FROM `isu` WHERE `jia_isu_uuid` = ?",
		jiaIsuUUID)
	if err != nil {
		c.Logger().Errorf("db error: %v", err)
		return c.NoContent(http.StatusInternalServerError)
...
func getIsuID(c echo.Context) error {
	...
	var res Isu
	err = db.Get(&res, "SELECT * FROM `isu` WHERE `jia_isu_uuid` = ?",
		jiaIsuUUID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
...
func getIsuGraph(c echo.Context) error {
	...
	var count int
	err = tx.Get(&count, "SELECT COUNT(*) FROM `isu` WHERE `jia_isu_uuid` = ?",
		jiaIsuUUID)
	if err != nil {
		c.Logger().Errorf("db error: %v", err)
		return c.NoContent(http.StatusInternalServerError)
...
func getTrend(c echo.Context) error {
	...
	for _, character := range characterList {
		...
		for _, isu := range isuList {
			conditions := []IsuCondition{}
			err = db.Select(&conditions,
				"SELECT * FROM `isu_condition` WHERE `jia_isu_uuid` = ? ORDER BY timestamp DESC LIMIT 2",
				isu.JIAIsuUUID,
			)
			if err != nil {
				c.Logger().Errorf("db error: %v", err)
